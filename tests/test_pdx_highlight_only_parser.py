...
from auditoria_pdf.parsing.document_parsers import PdxDocumentParser
from auditoria_pdf.parsing.highlight_text_extractors import fitz


@unittest.skipIf(fitz is None, "PyMuPDF no disponible")
class PdxHighlightOnlyParserTest(unittest.TestCase):
    def setUp(self) -> None:
        self.parser = PdxDocumentParser()

    def test_extracts_pdx_from_raw_text(self) -> None:
        pdf_path = self._build_pdf(
            body_text="Identificacion CC 12345678",
            highlighted_terms=["CC 12345678"],
        )
